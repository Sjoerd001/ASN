...
for IP in ${IP_LIST[*]}; do
	...
	status=0
	if [[ -n "$ipcheck" ]]; then
	    echo -e "\e[1m\e[31m${domain} - IP $IP is Blacklisted in RBL \e[0m" status code "$ipcheck"
	    status+=1
	fi
	[[ $status -eq 0 ]] && echo -e "\e[32m\e[1m${domain} - IP $IP is Not Blacklisted in RBL.... \e[0m" status code "$ipckeck"
    done
done
#//////////////////////////////////////////////////////////////////////////
##################################################### end of script ############################################################
